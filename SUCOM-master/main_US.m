...
for itr_outlier=2:size(store_R,2)
    ...
    for nr_run=1 %1:size(store_R,1)
        ...
        % Warm-up
        [~,  ~, ~, ~] = algorithm_US(pts_3d,pts_3d_,noise);

        % Running SUCOM for known-scale PCR
        tic;
        [scale_opt, R_opt, t_opt, ~] = algorithm_US(pts_3d,pts_3d_,noise);
        ...
        disp(['Scale error of SUCOM: ', num2str(s_error(nr_run,itr_outlier))]);
        disp(['Rotation error of SUCOM in degrees: ', num2str(R_error(nr_run,itr_outlier))]);
        disp(['Translation error of SUCOM in meters: ', num2str(t_error(nr_run,itr_outlier))]);
        disp(['Runtime of SUCOM in seconds: ', num2str(time(nr_run,itr_outlier))]);
        disp('  ');
        ...
        hold on;
        pcshow([pc2.Location(:,1),pc2.Location(:,2),pc2.Location(:,3)],[0 1 1],'MarkerSize',90);
        for i=round(n_ele*outlier_ratio)+1:n_ele
            plot3([pts_3d(i,1),pts_3d_(i,1)],[pts_3d(i,2),pts_3d_(i,2)],[pts_3d(i,3),pts_3d_(i,3)],'g','LineWidth',2);
        end
